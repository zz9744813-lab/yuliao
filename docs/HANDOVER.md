...
> 2026-09-20 Codex：长篇总方案已完成最终核对并原位定稿；现进入单场景 → 连续场景实现，逐步证据见 [Runtime 交接](runtime-handover-20260920.md) 与 [执行计划](plan.md)。
...
10. **语料扩产第四批**：EXP-0919-SCALE2，+209 L 帧（今日两批共 +431；
    L 主帧 ~1600，距"几千"目标过半）。
    ⚠ **2026-09-20：扩产池耗尽**——"未被任何实验用过+无水印+无拼音+≥60字+
    非番外"只剩 2 段（第五批 0 帧，`data/_dbg/scale_0920_20260920.log`）。
