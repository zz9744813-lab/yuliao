...
> **2026-09-23 00:2x 通道判定更新（主控证据 docs/K4_首轮真跑_证据_20260922.md；工作 agent 已核对落盘产物与其声明一致：out_k4_3/k4_paired.json live=true、3 包 n_techniques 全 0、failures=6 全 RuntimeFault、0 正文）**：22:55 探针的 502/上游 401 为**瞬态**——主控 00:0x 探针 deepseek-v4.1-flash/deepseek-flash 均 HTTP 200，**402 资金墙不成立**；真跑真卡点=中转 107.172.138.14:3000 对长文生成 120s 超时（gateway_http_500），高速通道 mc22.top:8899 实测 200/5.9s。主控已亲跑 K4 三场（A 臂包全空 ⇒ C3 不过；6 条失败中 s2/s3 四条为前场连锁幻影 world_revision_conflict）。P0 改判为「通道健康」（出处=主控文件，K5-A §2/任务单待拍板段的 402 表述待新派工统一更新，本轮不动）；修补任务已派 GUI 线（K2-A-LIVE-放量驱动=k2_extract_backfill.py 幂等/预算闸/--dry-run 零库写/--limit；k4 驱动 skipped_after_failure 标记；mc22 走 env 覆盖不改 .env + channel_changed=true）。另如实记：22:5x-23:0x 一次全量 pytest 瞬态红（后续连续两轮 767 例全绿，复现不出）与主控 live 实跑同机并发时间窗重合，日志已失不可归名，记悬案。
