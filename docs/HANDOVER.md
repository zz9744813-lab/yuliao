...
> **2026-09-22 17:0x K 系离线收官**：K0–K5 离线部分全部过会审——K3-A/K3-B 收口（77f6666/8173225），K2-A/K4-A 离线预置（93981b7），K5-A 评估 v1 定稿（940cd1d：成本口径改每调用基数——正常 160~240 万/最坏 480~720 万/止损 800 万；停策略 N≥8 下限；C5 未标 channel_changed 时 C1+C5 均不过；判据核验命令表；三场收据 out_k4_3/ 不可覆盖、10 场落 out_k4_10/）；58cf033 会审遗留 A1–A6 全清（ad9ee38：rollback_failed 并入本臂记录+回归测试、live 双闸 mock 前提钉死；940cd1d：术语统一/N 下限/C5 写死）。**唯一阻塞：deepseek 402 资金墙待拍板**——清单与拍板后一条命令见任务单待拍板段。
...
> **2026-09-23 00:2x 通道判定更新（主控证据 docs/K4_首轮真跑_证据_20260923.md；工作 agent 已核对落盘产物与其声明一致：out_k4_3/k4_paired.json live=true、3 包 n_techniques 全 0、failures=6 全 RuntimeFault、0 正文；**该首轮台账 6 条中 4 条为连锁幻影——按会审 89f779e R3 判定：此台账不可用于 C2「零失败臂」判定，仅作通道/空包证据**）**：22:55 探针的 502/上游 401 为**瞬态**——主控 00:0x 探针 deepseek-v4.1-flash/deepseek-flash 均 HTTP 200，**402 资金墙不成立**；真跑真卡点=中转 107.172.138.14:3000 对长文生成 120s 超时（gateway_http_500），高速通道 mc22.top:8899 实测 200/5.9s。主控已亲跑 K4 三场（A 臂包全空 ⇒ C3 不过；6 条失败中 s2/s3 四条为前场连锁幻影 world_revision_conflict）。P0 改判为「通道健康」（出处=主控文件，K5-A §2/任务单待拍板段的 402 表述待新派工统一更新，本轮不动）；修补任务已派 GUI 线（K2-A-LIVE-放量驱动=k2_extract_backfill.py 幂等/预算闸/--dry-run 零库写/--limit；k4 驱动 skipped_after_failure 标记；mc22 走 env 覆盖不改 .env + channel_changed=true）。另如实记：22:5x-23:0x 一次全量 pytest 瞬态红（后续连续两轮 767 例全绿，复现不出）与主控 live 实跑同机并发时间窗重合，日志已失不可归名——**按会审 89f779e R6 不作结案归档，记待办：live 实跑与全量 pytest 禁止同机并发窗重叠，守卫机制待落（落实前每次全绿结论须注明当窗是否有并发实跑）**。
  【2026-09-23 R6 守卫已落（9e02916 会审 7 条整改后）】app/live_guard.py：锁文件 O_EXCL 原子互斥；**两侧同锁口径**——pytest 侧（conftest 统一调用共享 refuse_if_live_running）盯**生产锁位** repo/data/live_run.lock（盯被 conftest 覆写的测试 DATA_DIR 会恰好看不到生产 live 的锁，会审严重项修正）；保守释放（回读比对 pid+purpose 才删，不删接管者的锁）；损坏锁按存在处理宁拦不猜、死锁人工核清；k2/k4 --live 接线各有「锁在→SystemExit 且未发起调用」回归；边界如实声明：守卫覆盖默认生产 DATA_DIR 的 live。tests/test_live_guard.py 6 回归 + 接线 2。
...
**审查 A11 已修（2026-09-21 午后，P2）**：多段基准 context 的隔离
空隙——context 只按整段哈希时，「段落A\n\n段落B」的基准 context 里
段落 A 单独作为训练目标/前文不命中整段哈希（审查 70 字隔离复现实测
放行）。修复：_bench_hashes 对 context 的**组成段落各自入哈希集**
（≥50 字口径与训练侧一致；a/b 两侧保持无长度下限原口径——误加下限
会挤掉 365 个短哈希，实测 1971→1606、twin 排除测试打红，已回滚该
半成品）。verify_training_export 自动继承（同一哈希集）。回归：审查
复现测（段落 A 单独作目标必被逮+干净行不误伤）+ 既有 twin 测试；
...
/ 1 项 P2（A10）。
**审查 A10 已修（2026-09-21 午后，P2，LG 侧审查项全清）**：核验器
抽取错误不再烧 Writer 修稿额度。事故（审查隔离复现）：正文与计划事件
一字未动，仅核验器把未变化的 lamp.lit=false 多列进 changes →
state_patch_not_authorized_by_plan 被当正文缺陷 → 3 Writer + 3
Verifier 耗尽修稿额度（上一阶段第三场也曾因两枚未变化的硬币被误列
补丁而多改一次正文）。修复：pipeline 区分**正文事实缺陷**与**核验
工件缺陷**——正文零缺陷信号（无 hard issue、无长度越界）+ 只有补丁
清单失配 = 工件缺陷 → 走**有上限核验返修**（verifier.N.state1，正文
一字不动、只修 changes 清单）；返修不了如实失败
（verifier_state_repair_exhausted），绝不静默吞掉未经确认的状态变化、
绝不为核验器的错改正文；真正文缺陷（含 hard 信号）照旧走 Writer 修
稿。回归 3 测（监督口径「正确正文+错误核验」）：误列未变化事实→
Writer 只发 1 次+返修一轮即过+lamp 不进正史补丁+审计 ok；返修仍
回毒→如实失败零提交；正文真缺变化+hard issue→照旧烧满修稿轮（不
误伤旧路径）。既有 forged 安全测试随新语义更新：伪造 after 不得授信
...
（拍板点见 §12 第 2 项）。
**A01 二轮收紧（2026-09-21 午后，知识化调整方案 §1.1 口径）**：
方案 23:14 复现旧客户端仍可误译——A01 首版给无 pid 提交留了「按最近
一次持久化呈现回退」的猜义路径。二轮删除两条回退（进程内最近映射 +
DB 最近呈现）：**凡带 A/B 语义（winner A/B 或有批注）的提交必须呈现
绑定**——该题存在任何呈现行而无 pid → 409 拒收（判定不落库，客户端
重取题重提）；完全没有呈现行的 pre-A01 历史题保留「存原始值」
unresolved 路径（如实存未知，不是猜）。重启语义改由 **pid 绑定跨重启
存活**承担（呈现行在 DB，进程内缓存无关紧要——回归：清映射后带原
pid 改判照常翻对）。测试随新契约重写（回退测→409 拒收测+pid 对照、
重启测→pid 存活、改判删呈现行→404 绑定失效；测试助手与真实前端
一致自动携带 pid）。生产库的 review_presentations 表由 init_db/
_migrate 在下次服务启动时落表（方案「合入≠生效」点已覆盖）。全量
674 例全绿（退出码 0）。
**K0-C 收口（2026-09-21）**：bal-v2 终报与 Freeze 已成文——
[bal-v2-final-report-20260921.md](bal-v2-final-report-20260921.md)：
冻结对象 BS-5543d4b7ac4c（180 题/95 源段，原始分母全量给出）；
三评委全报（qoder 0.839/kimi 0.828 达标、agnes 0.728 未达；
CI/置换/分层/留一全表）；N4 长度基线 0.500 三家全碾压；类型混杂
与未完成子检验显式标注（每类型分层、跨语料、人类对照、跨代趋势
