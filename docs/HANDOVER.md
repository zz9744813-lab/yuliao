...
9. **子基准全天读数汇总**（均通过否掉检验，provisional→升级）：
   · **nat-v1 自然度轴四模型收敛**：qoder 0.915 / kimi 0.910 / **agnes 0.910** /
     deepseek 0.879——置换 p≈0、留一波动 ≤0.01、无位置偏差；仍受 15 段聚类上限。
   · **hvai-v1（人 vs 自由重建）全量**：qoder **0.827**（429/519，26 题未答），
     **519 段各 1 题**（聚类健康），置换 p≈0，pass。冒烟首 60 题 0.912 偏高是
     取样顺序效应——以全量 0.827 为准。
   · 读法：身份判别 0.83 / 自然度 0.91 都远高于盲评 κ≈0.1 + 控制臂 77% 偏 AI
     ——评委**读得出**差异、**偏好**反向，三条证据链闭合（§0.5③ 定量版）。
...
13. **运维**：评审台服务 8787 曾掉线，10:00 已重启（`data/_dbg/serve_20260919_1000.log`，
