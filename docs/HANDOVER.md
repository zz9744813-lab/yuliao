...
**审查 A03 已修（同日深夜）**：训练导出重导 + 验收器落地。旧导出
（09-19 文件）实测 SFT/Rewrite 各 114 行与基准文本重合、32 行源未校勘、
RM 172 组同源同文不同分。处置：①现行代码重导三件套——writer_sft_v3
（2037 行/1672 源段，排除基准段 873/坏源 159）、rewrite_v2（2037/1672）、
rm_v1（573/150，冲突消解丢弃 827 行、排除基准 362/坏源 207）；②新增
scripts/verify_training_export.py：按审查复算口径（忽略空白、≥50 字、
目标+前文，含 dict 形态前文）独立重算基准重合、RM 同源同文多分组数、
主键缺失、源质量（integrity.src_ok 回连复核），旁挂生成
<file>.manifest.json（sha256+行数+源段数+各项计数）；③**训练入口契约
accept_for_training()**：无 manifest（旧导出/手生成）、sha256 不符
（重导前旧文件/被改动）、验收未过 → 一律拒收；④同源不相加：SFT 与
Rewrite 是同源段两种用法，合计按不同源段数（1672）算，不许按行数
相加（manifest 有注记）。三件重导出验收全 PASS（重合 0/冲突 0/缺键 0/
未校勘 0），manifest 落 data/exports/（gitignore 外置，可重跑验收器
再生）。回归 5 测（重合含 dict 前文必逮、RM 冲突组、缺键、训练入口
三态：接受/无清单拒/改动拒、源质量计数）。
审查剩余：5 项 P1 / 3 项 P2 排队中。
