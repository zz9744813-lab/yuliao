...
- 用户本轮明确只要求根据最新情况写新计划；已落盘 [知识化调整方案](Language_Genome_知识化调整方案_20260920.md)，以其 K0–K5 接续，不重新打开已完成的第 6、7 项。
- 23:14 只读基线为本机 main `97fec01`：原运行库仍为 3 个提交 / 14 次调用；本轮没有调用 Writer 或 Verifier，没有更改正文或收据。Runtime 35 项 + 新盲评呈现 5 项定向回归通过，不代表当前全仓已复跑。
- 已考虑并行修复：A02 工作区版本在原 3 题 / 9 样本复现中无错位；Distiller A08 的三个竞争用例通过。A01 新客户端已修，但无 presentation_id 的旧页仍可被最新映射翻转；被检查的研究库尚未出现新呈现表，部署生效不能只看 commit。
- 调整重点：兼容旧数据的条件化策略与证据 → 自动 Query → 每场独立冻结包 → 新目录内真实对照。原 CLI 当前是三场共用一包，迁移必须保持旧断点及收据可读。A09 / A10 仍为真实 A/B 的前置修复。
- 证据目录：`../../outputs/genome-adjustment-20260920/`。已更新 README、总交接、plan 与总方案入口；本轮没有实施 v2、迁移正式库、开始付费实验或停止在途任务。
