...
def _blind_ensure_loaded() -> None:
    ...
    **后果不是 409**（2026-09-26 独立审查席更正、主控复核成立）：有效 pid 的
    ReviewPresentation 行在端题时已 commit，提交路径的 DB 兜底会重建 served 并 200；
    真实后果是 (1) 指纹证伪 `a_hash`/`b_hash` 被**静默跳过**（兜底重建的 served 不含这两键，
    `:1097` 的条件恒假），(2) 改判入口 `review_serve_one` 无 DB 兜底 ⇒ 旧判定翻不回 A/B。
    单 worker/测试下看不出来，多线程部署下是真缺陷。
    现在读路径要么拿到旧一整套、要么拿到新一整套。
    """
