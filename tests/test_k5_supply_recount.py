...
# ── 夹具设计（期望值手算钉死）─────────────────────────────────────────
# 口径 A（IS TRUE ∧ 非benchmark ∧ 白名单非空）命中 5 段：
#   s_t1 s_t2 s_l1(数字1漏入) s_w6a s_w6b
# 口径 A 严格布尔命中 4 段：s_t1 s_t2 s_w6a s_w6b（排除 s_l1）
# 口径 B（非benchmark ∧ 白名单非空）命中 12 段：W1 除 s_b1 外 10 段 + W6 2 段
...
WORK_SOURCES = [
    ("W1", json.dumps(["train", "judge"])),
    ("W2", json.dumps([])),            # 空数组 → A/B 排除
    ("W4", "not-json{{"),              # 非法 JSON → fail-closed 排除
    ("W5", json.dumps({"a": 1})),      # 合法但非数组 → 排除
    ("W6", json.dumps(["train"])),
]
...
SEGMENTS = [
    ...
    ("s_w2", "W2", '{"src_ok": true}', None),        # 空白名单
    ("s_w3", "W3", '{"src_ok": true}', None),        # 无 work_sources 登记行
    ("s_w4", "W4", '{"src_ok": true}', None),        # 白名单非法 JSON
    ("s_w5", "W5", '{"src_ok": true}', None),        # 白名单非数组
...
def make_db(tmp_path: Path) -> Path:
    # 同测试内多次建库互不冲突（tmp_path 复用）
    make_db._seq = getattr(make_db, "_seq", 0) + 1
    p = tmp_path / f"fixture_supply_{make_db._seq}.db"
    ...
    con.executemany(
        "INSERT INTO segments(id, work_id, ordinal, text, integrity, role) "
        "VALUES (?,?,?,?,?,?)",
